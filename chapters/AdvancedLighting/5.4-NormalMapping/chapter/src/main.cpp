...
int main()
{
	...
	std::shared_ptr<Shader> litNormalShader = std::make_shared<Shader>("shaders/LitNormal.vert", "shaders/LitNormal.frag");
	...
	std::shared_ptr<Texture> diffuseBrickTexture = std::make_shared<Texture>(std::string(ASSETS_PATH) + "/brick/brickwall.jpg", 2);
	std::shared_ptr<Texture> normalBrickTexture = std::make_shared<Texture>(std::string(ASSETS_PATH) + "/brick/brickwall_normal.jpg", 3);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		{
			litNormalShader->Bind();

			glm::mat4 model = glm::mat4(1.0f);
			
			// rotate the quad to show normal mapping from multiple directions
			model = glm::rotate(model, glm::radians((float)glfwGetTime() * -10.0f), glm::normalize(glm::vec3(1.0, 0.0, 1.0)));
			litNormalShader->SetMat4("model", model);

			diffuseBrickTexture->BindAndActivateUnit();
			normalBrickTexture->BindAndActivateUnit();
			litNormalShader->SetInt("diffuseMap", diffuseBrickTexture->GetTextureUnit());
			litNormalShader->SetInt("normalMap", normalBrickTexture->GetTextureUnit());
			litNormalShader->SetDirectionalLight(directionalLight, "directionalLight", directionalLight.GetTransform());
