...
int main()
{
	...
	std::shared_ptr<Shader> combinedHDRandBloomPostProcessShader = std::make_shared<Shader>("shaders/CombinedHDRandBloomPostProcess.vert", "shaders/CombinedHDRandBloomPostProcess.frag");

	std::shared_ptr<Shader> gaussianBlurPostProcessShader = std::make_shared<Shader>("shaders/GaussianBlurPostProcess.vert", "shaders/GaussianBlurPostProcess.frag");
	...
	spec.Attachments = { FramebufferTextureFormat::RGBA16F, FramebufferTextureFormat::RGBA16F, FramebufferTextureFormat::DEPTH24STENCIL8 };
	Framebuffer2 BloomHDRFramebufferTarget(spec);

	FramebufferSpecification pingPongSpec;
	pingPongSpec.Width = g_WindowWidth;
	pingPongSpec.Height = g_WindowHeight;
	pingPongSpec.Attachments = { FramebufferTextureFormat::RGBA16F };
	Framebuffer2 IntermediateFramebufferTargetHorizontal(pingPongSpec);
	Framebuffer2 IntermediateFramebufferTargetVertical(pingPongSpec);

	constexpr int NormalColorAttachmentIndex = 0;
	constexpr int BloomColorAttachmentIndex = 1;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		BloomHDRFramebufferTarget.Bind();
		...
		BloomHDRFramebufferTarget.Unbind();
		...
		constexpr int numBloomIterations = 10;
		gaussianBlurPostProcessShader->Bind();
		Framebuffer2& targetBlurredFramebuffer = IntermediateFramebufferTargetHorizontal;
		for (uint32_t i = 0; i < numBloomIterations; ++i)
		{
			int colorAttachmentIndex = 0;
			targetBlurredFramebuffer = bHorizontal ? IntermediateFramebufferTargetHorizontal : IntermediateFramebufferTargetVertical;

			gaussianBlurPostProcessShader->SetInt("horizontal", bHorizontal);
			targetBlurredFramebuffer.Bind();

			if (bFirstIteration)
			{
				// bright areas
				BloomHDRFramebufferTarget.BindColorAttachment(BloomColorAttachmentIndex);
				targetBlurredFramebuffer = BloomHDRFramebufferTarget;
				colorAttachmentIndex = 1;
			}
			else
			{
				targetBlurredFramebuffer.BindColorAttachment();
			}

			gaussianBlurPostProcessShader->SetInt("image", targetBlurredFramebuffer.GetColorAttachmentTextureUnit(colorAttachmentIndex));
			Renderer::Draw(postProcessQuadVAO, gaussianBlurPostProcessShader);
		...
		combinedHDRandBloomPostProcessShader->Bind();

		//normal scene
		BloomHDRFramebufferTarget.BindColorAttachment(NormalColorAttachmentIndex);

		// blurred bright areas
		targetBlurredFramebuffer.BindColorAttachment(NormalColorAttachmentIndex, 29);

		combinedHDRandBloomPostProcessShader->SetInt("scene", BloomHDRFramebufferTarget.GetColorAttachmentTextureUnit(NormalColorAttachmentIndex));
		combinedHDRandBloomPostProcessShader->SetInt("bloomBlur", targetBlurredFramebuffer.GetColorAttachmentTextureUnit(31 - 29));
		combinedHDRandBloomPostProcessShader->SetFloat("exposure", s_Exposure);
		Renderer::Draw(postProcessQuadVAO, combinedHDRandBloomPostProcessShader);
