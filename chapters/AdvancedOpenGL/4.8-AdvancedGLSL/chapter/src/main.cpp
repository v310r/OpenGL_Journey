...
int main()
{
	...
	std::shared_ptr<Shader> pointShader = std::make_shared<Shader>("shaders/Point.vert", "shaders/Point.frag");

	std::shared_ptr<Shader> fragCoordExampleShader = std::make_shared<Shader>("shaders/FragCoordExample.vert", "shaders/FragCoordExample.frag");
	
	std::shared_ptr<Shader> frontFacingExampleShader = std::make_shared<Shader>("shaders/FrontFacingExample.vert", "shaders/FrontFacingExample.frag");
	...
	SSimpleCube pointCubeEntity;
	{
		pointCubeEntity.Transform.Translation = glm::vec3(0.0f, platformEntity.Transform.Translation.y + 0.55f, 0.0f);
		pointCubeEntity.Transform.Scale = glm::vec3(1.0f, 1.0f, 1.0f);

		pointCubeEntity.ShaderObject = pointShader;

		pointCubeEntity.VAO = cubeVAO;
	}

	SSimpleCube fragCoordCubeEntity;
	{
		fragCoordCubeEntity.Transform.Translation = glm::vec3(2.0f, platformEntity.Transform.Translation.y + 4.0f, -3.0f);
		fragCoordCubeEntity.Transform.Scale = glm::vec3(1.0f, 1.0f, 1.0f);

		fragCoordCubeEntity.ShaderObject = fragCoordExampleShader;

		fragCoordCubeEntity.VAO = cubeVAO;
	}

	std::shared_ptr<Texture> frontTexture = std::make_shared<Texture>(std::string(ASSETS_PATH) + "/wooden_container.jpg", 0, false);
	std::shared_ptr<Texture> backTexture = std::make_shared<Texture>(std::string(ASSETS_PATH) + "/cube.jpg", 1, false);
	SSimpleCube frontFacingCubeEntity;
	{
		frontFacingCubeEntity.Transform.Translation = glm::vec3(2.0f, platformEntity.Transform.Translation.y + 7.0f, -3.0f);
		frontFacingCubeEntity.Transform.Scale = glm::vec3(1.0f, 1.0f, 1.0f);

		frontFacingCubeEntity.ShaderObject = frontFacingExampleShader;

		frontFacingCubeEntity.VAO = cubeVAO;
	}
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// CUBE WITH POINT PRIMITIVE DRAWING
		{
			pointCubeEntity.ShaderObject->Bind();

			glm::mat4 pointCubeModel = UtilityFunctions::CalculateTransformMatrix(pointCubeEntity.Transform);
			pointCubeEntity.ShaderObject->SetMat4("model", pointCubeModel);
			pointCubeEntity.ShaderObject->SetMat3("normalMatrixTransform", glm::transpose(glm::inverse(glm::mat3(Renderer::GetView() * pointCubeModel))));

			Renderer::Draw(pointCubeEntity.VAO, pointCubeEntity.ShaderObject, SDrawSettings{EDrawPrimitiveMode::Points});
		}
		
		// CUBE WITH FRAG COORD EXAMPLE
		{
			fragCoordCubeEntity.ShaderObject->Bind();

			glm::mat4 fragCoordCubeModel = UtilityFunctions::CalculateTransformMatrix(fragCoordCubeEntity.Transform);
			fragCoordCubeEntity.ShaderObject->SetMat4("model", fragCoordCubeModel);
			fragCoordCubeEntity.ShaderObject->SetMat3("normalMatrixTransform", glm::transpose(glm::inverse(glm::mat3(Renderer::GetView() * fragCoordCubeModel))));

			Renderer::Draw(fragCoordCubeEntity.VAO, fragCoordCubeEntity.ShaderObject, SDrawSettings{ EDrawPrimitiveMode::Triangles });
		}

		// CUBE WITH FRONT FACING EXAMPLE
		{
			frontFacingCubeEntity.ShaderObject->Bind();

			frontTexture->BindAndActivateUnit();
			backTexture->BindAndActivateUnit();

			frontFacingCubeEntity.ShaderObject->SetInt("frontTexture", frontTexture->GetTextureUnit());
			frontFacingCubeEntity.ShaderObject->SetInt("backTexture", backTexture->GetTextureUnit());


			glm::mat4 frontFacingCubeModel = UtilityFunctions::CalculateTransformMatrix(frontFacingCubeEntity.Transform);
			frontFacingCubeEntity.ShaderObject->SetMat4("model", frontFacingCubeModel);
			frontFacingCubeEntity.ShaderObject->SetMat3("normalMatrixTransform", glm::transpose(glm::inverse(glm::mat3(Renderer::GetView() * frontFacingCubeModel))));

			Renderer::Draw(frontFacingCubeEntity.VAO, frontFacingCubeEntity.ShaderObject, SDrawSettings{ EDrawPrimitiveMode::Triangles });
		}
