...
int main()
{
	...
	std::shared_ptr<VertexArray> houseVAO = std::make_shared<VertexArray>();
	{
		std::shared_ptr<VertexBuffer> houseVertexBuffer = std::make_shared<VertexBuffer>(house2DVertices, sizeof(house2DVertices) / sizeof(house2DVertices[0]));
		houseVertexBuffer->SetLayout(BufferLayout
		{
			{ShaderUtility::EShaderDataType::Float2, "aPos"},
			{ShaderUtility::EShaderDataType::Float3, "aColor"}
		});

		houseVAO->AddVertexBuffer(houseVertexBuffer);
	...
	std::shared_ptr<Shader> houseShader = std::make_shared<Shader>("shaders/House2D.vert", "shaders/House2D.frag", "shaders/House2D.geom");

	std::shared_ptr<Shader> explodeShader = std::make_shared<Shader>("shaders/Explode.vert", "shaders/Explode.frag", "shaders/Explode.geom");

	// for debugging normals
	std::shared_ptr<Shader> normalDebugShader = std::make_shared<Shader>("shaders/NormalDebug.vert", "shaders/NormalDebug.frag", "shaders/NormalDebug.geom");
	...
	SSimpleCube houses2D;
	{
		houses2D.ShaderObject = houseShader;

		houses2D.VAO = houseVAO;
	}

	std::shared_ptr<Entity> explodedBackpackEntity = std::make_shared<Entity>();
	{
		STransform backpackTransform;
		backpackTransform.Translation = glm::vec3(5.0f, 0.0f, 0.0f);
		explodedBackpackEntity->SetTransform(backpackTransform);

		explodedBackpackEntity->SetScale(glm::vec3(1.0f, 1.0f, 1.0f));

		explodedBackpackEntity->AddMesh(std::string(ASSETS_PATH) + "/backpack/backpack.obj", explodeShader);
	}

	std::shared_ptr<Entity> litBackpackEntity = std::make_shared<Entity>();
	{
		STransform backpackTransform;
		backpackTransform.Translation = glm::vec3(-5.0f, 0.0f, 0.0f);
		litBackpackEntity->SetTransform(backpackTransform);

		litBackpackEntity->SetScale(glm::vec3(1.0f, 1.0f, 1.0f));

		litBackpackEntity->AddMesh(std::string(ASSETS_PATH) + "/backpack/backpack.obj", meshLitShader);
	}

	std::shared_ptr<Mesh> normalDebugMesh = std::make_shared<Mesh>(std::string(ASSETS_PATH) + "/backpack/backpack.obj", normalDebugShader);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// 2D houses
		{
			Renderer::Draw(houses2D.VAO, houses2D.ShaderObject, SDrawSettings{ EDrawPrimitiveMode::Points });
		}
		...
		// Exploding entity
		{
			...
			glm::mat4 backpackModel = UtilityFunctions::CalculateTransformMatrix(explodedBackpackEntity->GetTransform());
			explodeShader->SetMat4("model", backpackModel);
			explodeShader->SetFloat("time", glfwGetTime());
			explodeShader->SetMat3("normalMatrixTransform", glm::transpose(glm::inverse(glm::mat3(Renderer::GetView() * backpackModel))));

			Renderer::Draw(explodedBackpackEntity);
		}
		...
		// for debugging normals
		{
			...
			meshLitShader->SetDirectionalLight(directionalLight, "directionalLight", directionalLight.GetTransform());

			glm::mat4 debugBackpackModel = UtilityFunctions::CalculateTransformMatrix(litBackpackEntity->GetTransform());
			meshLitShader->SetMat4("model", debugBackpackModel);
			meshLitShader->SetMat3("normalMatrixTransform", glm::transpose(glm::inverse(glm::mat3(Renderer::GetView() * debugBackpackModel))));

			Renderer::Draw(litBackpackEntity);

			normalDebugShader->Bind();
			normalDebugShader->SetMat4("model", debugBackpackModel);
			normalDebugShader->SetMat3("normalMatrixTransform", glm::transpose(glm::inverse(glm::mat3(Renderer::GetView() * debugBackpackModel))));

			Renderer::Draw(normalDebugMesh);
		}
