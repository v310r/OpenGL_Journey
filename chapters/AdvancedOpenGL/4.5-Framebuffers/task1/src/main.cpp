...
void DrawScene(const SCube& cubeEntity, const std::shared_ptr<VertexArray>& cubeVAO, const SQuad& platformEntity, const std::shared_ptr<VertexArray>& quadVAO,  const std::shared_ptr<Shader>& litShader, const DirectionalLight& directionalLight, const std::vector<glm::vec3>& cubePositions)
{
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// draw normal version
		DrawScene(cubeEntity, cubeVAO, platformEntity, quadVAO, litShader, directionalLight, cubePositions);

		Renderer::EndScene();

		FlyCamera mirroredCamera = g_Camera;
		mirroredCamera.SetYaw(-g_Camera.GetYaw());

		Renderer::BeginScene(mirroredCamera);
		postProcessFramebuffer.Bind();

		RenderCommand::SetClearColor(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
		RenderCommand::Clear();

		// draw mirrored version as small window in top right corner of the screen
		DrawScene(cubeEntity, cubeVAO, platformEntity, quadVAO, litShader, directionalLight, cubePositions);

		postProcessFramebuffer.Unbind();

		// not needed for this example, but might be useful in other cases, when you render more than one quad or whatever
		glDisable(GL_DEPTH_TEST);

		postProcessShader->Bind();
		postProcessFramebuffer.BindColorAttachment();
		postProcessShader->SetInt("screenTexture", postProcessFramebuffer.GetColorAttachmentUnit());
