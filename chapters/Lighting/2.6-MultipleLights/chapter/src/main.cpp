	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// POINT LIGHTS
		{
			for (unsigned int i = 0; i < 4; ++i)
			{
				...
				litShader->SetFloat3(pointLight + ".position", glm::mat4(view)* glm::vec4(pointLightPositions[i], 1.0f));
		...
		// SPOTLIGHT
		{
			...
			litShader->SetFloat3("spotlight.position", glm::mat4(view)* glm::vec4(g_Camera.GetPosition(), 1.0f));
