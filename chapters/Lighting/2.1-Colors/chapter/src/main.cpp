...
int main()
{
	...
	{
		std::shared_ptr<VertexBuffer> vb = std::make_shared<VertexBuffer>(g_CompactCubeVertices, g_CompactCubeVerticesSizeInBytes / sizeof(g_CompactCubeVertices[0]));
		vb->SetLayout(BufferLayout
		{
			{ShaderUtility::EShaderDataType::Float3, "aPos"}
		});

		cubeVAO->AddVertexBuffer(vb);
	...
	SCube cubeEntity;
	cubeEntity.Color = glm::vec3{0.0f, 1.0f, 0.0f};
	...
	{
		std::shared_ptr<VertexBuffer> vb = std::make_shared<VertexBuffer>(g_CompactCubeVertices, g_CompactCubeVerticesSizeInBytes);
		vb->SetLayout(BufferLayout
		{
			{ShaderUtility::EShaderDataType::Float3, "aPos"}
		});

		lightVAO->AddVertexBuffer(vb);

		std::shared_ptr<IndexBuffer> indexBuffer = std::make_shared<IndexBuffer>(g_CubeIndices, g_CubeIndicesSizeInBytes / sizeof(g_CubeIndices[0]));
		lightVAO->SetIndexBuffer(indexBuffer);
	}

	std::shared_ptr<Shader> lightingShader = std::make_shared<Shader>("shaders/lighting.vert", "shaders/lighting.frag");
	lightingShader->Bind();
	lightingShader->SetFloat3("color", lightCubeEntity.Color);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (ImGui::CollapsingHeader("Cube entity", ImGuiTreeNodeFlags_None))
		{
			...
			{
				ImGui::SliderFloat("Translate X", &cubeEntity.Transform.Translation.x, -1.0f, 1.0f);
				ImGui::SliderFloat("Translate Y", &cubeEntity.Transform.Translation.y, -1.0f, 1.0f);
				ImGui::SliderFloat("Translate Z", &cubeEntity.Transform.Translation.z, -1.0f, 1.0f);
		...
		if (ImGui::CollapsingHeader("Light source", ImGuiTreeNodeFlags_None))
		{
			...
			if (ImGui::TreeNode("Transform"))
			{
				ImGui::SliderFloat("Translate X", &lightCubeEntity.Transform.Translation.x, -1.0f, 1.0f);
				ImGui::SliderFloat("Translate Y", &lightCubeEntity.Transform.Translation.y, -1.0f, 1.0f);
				ImGui::SliderFloat("Translate Z", &lightCubeEntity.Transform.Translation.z, -1.0f, 1.0f);
