...
void main()
{
	...
	vec3 diffuse = (diffuseValue * sampledDiffuseValue) * light.diffuse * light.intensity;
	...
	vec3 specular = sampledSpecularValue * cameraBasedSpecularValue * light.intensity;
	...
	const float distance = length(light.position - FragmentPosition);
	const float attenuation = 1.0f / (light.constant + light.linear * distance + light.quadratic * (distance * distance));
