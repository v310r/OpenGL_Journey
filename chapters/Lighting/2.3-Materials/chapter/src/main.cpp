...
int main()
{
	...
	cubeEntity.MaterialData = std::make_shared<SimpleMaterial>(cubeShader);;

	cubeEntity.MaterialData->SetAmbient(glm::vec3{ 1.0f, 0.5f, 0.31f });
	cubeEntity.MaterialData->SetDiffuse(glm::vec3{ 1.0f, 0.5f, 0.31f });
	cubeEntity.MaterialData->SetSpecular(glm::vec3{ 0.5f, 0.5f, 0.5f });
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (ImGui::CollapsingHeader("Cube entity", ImGuiTreeNodeFlags_None))
		{
			...
			{
				ImGui::ColorEdit3("Ambient RGB", glm::value_ptr(cubeEntity.MaterialData->GetAmbient()));
				ImGui::ColorEdit3("Diffuse RGB", glm::value_ptr(cubeEntity.MaterialData->GetDiffuse()));
				ImGui::ColorEdit3("Specular RGB", glm::value_ptr(cubeEntity.MaterialData->GetSpecular()));
