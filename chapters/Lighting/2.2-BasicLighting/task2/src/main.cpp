...
int main()
{
	...
	cubeShader->SetFloat3("lightPosition", lightCubeEntity.Transform.Translation);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		cubeShader->SetMat3("normalMatrixTransform", glm::transpose(glm::inverse(glm::mat3(cubeModel))));
		...
		cubeShader->SetFloat("ambientStrength", lightCubeEntity.AmbientStrength);
		cubeShader->SetFloat("specularStrength", lightCubeEntity.SpecularStrength);
		...
		if (ImGui::CollapsingHeader("Light source", ImGuiTreeNodeFlags_None))
		{
			...
			if (ImGui::TreeNode("Light properties"))
			{
				...
				ImGui::SliderFloat("Ambient Strength", &lightCubeEntity.AmbientStrength, 0.0f, 1.0f);
				ImGui::SliderFloat("Specular Strength", &lightCubeEntity.SpecularStrength, 0.0f, 1.0f);
