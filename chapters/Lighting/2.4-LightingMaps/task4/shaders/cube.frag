...
void main()
{
	...
	const vec3 specular = sampledSpecularValue * cameraBasedSpecularValue * light.specular;

	vec3 emission = vec3(0.0f);
	if (sampledSpecularValue.r == 0.0f)
	{
		// better solution is to remake the texture to fit into wooden part
		emission = vec3(texture(material.emission, TextureCoordinates + vec2(0.0f, time)));
		emission = emission * min(((sin(time * 10) * 0.5f + 0.5f) * 2), 1.0f);
	}
